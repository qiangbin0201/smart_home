...
//	}
    ...
    public static Bitmap resizeBitmap(Bitmap bm) {
        ...
        int width = 300;
        width = width > bm.getWidth() ? bm.getWidth() : width;
        int height = width * bm.getHeight() / bm.getWidth();
        return resizeBitmap(bm, width, height, false);
        // return resizeBitmap(bm, 200, 200, false);
    }

    public static Bitmap resizeBitmap(Bitmap bm, int width) {
        if (bm == null || bm.isRecycled()) {
            return null;
        }
...
//			return resize(tmp, newWidth, newHeight);
    ...
    public static Bitmap scaleBitmapImgByWidth(Bitmap img, int scaleWidth) {
        Matrix matrix = new Matrix();
        float widthRatio = ((float) scaleWidth / img.getWidth());
    ...
    public static String getImageSizeURL(ImageSize imageSize, String url) {
        ...
        if (!TextUtils.isEmpty(url)) {
            if (!url.contains("cimage.zaikan.mobi.sinastorage.com")) {
                return url;
            }
            ...
            } else if (imageSize == ImageSize.RESIZE) {
                return url.substring(0, url.length() - 1) + "3";
            } else {
    ...
    public static String getFitSizeImageURL(String url, int width) {
        ImageUtils.ImageSize imageSize = width > 640 ? ImageUtils.ImageSize.RESIZE : ImageUtils.ImageSize.ORIGIN;
        return getImageSizeURL(imageSize, url);
    }
