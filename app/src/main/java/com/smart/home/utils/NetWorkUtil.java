...
//                proxyProperty = new String(Base64.encode(authentication.getBytes()));
                proxyProperty = new String(EncryptUtils.base64Encode(authentication.getBytes()));
                proxyProperty = " Basic " + proxyProperty;
                // uc.setRequestProperty("Proxy-Authorization", " Basic " + encodedLogin);
            }
        } catch (Exception e) {
            // 适配4.2以上系统，默认无法读取apn设置的异常
        } finally {
